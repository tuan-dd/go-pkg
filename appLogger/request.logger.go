...
func (logger *Logger) ReqClientLog(reqCtx *common.ReqContext, method, path string) {
	logger.Info(fmt.Sprintf(`Accepted Request [%s/%s] - %s]`, method, path, reqCtx.CID), zap.Object("data", ReqClientLogger{
		CID:       reqCtx.CID,
		Method:    method,
		Url:       path,
		IP:        reqCtx.IP,
...
func (logger *Logger) ResClientLog(reqCtx *common.ReqContext, statusCode uint, err error) {
	...
	if err != nil {
		...
		logger.Info(fmt.Sprintf(`Response [%s] - %d - %s]`, reqCtx.CID, statusCode, duration), zap.Object("data", data))
	}
}
