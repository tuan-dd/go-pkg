...
func DefaultSchedulerCfg(logger *appLogger.Logger) *asynq.SchedulerOpts {
...
func NewConnect(cfg *QueueConfig, logger *appLogger.Logger) (*Connection, *response.AppError) {
...
func (c *Connection) Run() *response.AppError {
	...
	if err := c.server.Run(c.Queue); err != nil {
